...
export async function saveResourceHelper(dataResource: SchemaResources, supabase: SupabaseClient) {
    ...
    const dataDetail = await saveDetaislResources(dataResource, supabase, data?.id)
    return dataDetail
}

async function saveDetaislResources(dataResource: SchemaResources, supabase: SupabaseClient, idResource: string) {
    const titleDetails = dataResource.detail_title
    const descriptionDetails = dataResource.detail_desciption

    if (!idResource) {
        throw new Error("Se requiere de un identificador de recurso para los datelles")
    }
    ...
    const { data, error } = await supabase.from("details_resources").insert(formatedDetails).select("id").single()
