...
export async function getServerUser() {
  ...
  const { data: profileData, error: profileError } = await getUserProfileQuery(id, supabaseServer);
  ...
  return {
    ...
    profile_public: profileData.user_profile_preferences?.profile_public ?? true,
    allow_email: profileData.user_profile_preferences?.allow_email ?? false,
    allow_whatsapp: profileData.user_profile_preferences?.allow_whatsapp ?? false,
    reputation: {
      questions: profileData.questions?.[0]?.count ?? 0,
      responses: profileData.question_comments?.[0]?.count ?? 0,
      score: "bronze",
      achievement: {
        achievement_id: profileData.user_achievements?.[0]?.achievement_id ?? null,
      },
    },
  };
}
